...
class Memory(Check):
    ...
    def check(self, agentConfig):
        ...
        try:
            ...
            self.logger.info('Missing Win32_PerfFormattedData_PerfOS_Memory. No memory metrics will be returned.')
            return self.get_metrics()
...
class Network(Check):
    ...
    def check(self, agentConfig):
        ...
        for iface in self.wmi_sampler:
            name = iface.get('Name')
            bytesreceivedpersec = iface.get('BytesReceivedPerSec')
            bytessentpersec = iface.get('BytesSentPerSec')

            name = self.normalize_device_name(name)
            if bytesreceivedpersec is not None:
                self.save_sample('system.net.bytes_rcvd', bytesreceivedpersec,
                                 device_name=name)
            if bytessentpersec is not None:
                self.save_sample('system.net.bytes_sent', bytessentpersec,
                                 device_name=name)
        return self.get_metrics()
